...
function GenerateAffirmationPage() {
  ...
  useEffect(() => {
    if (!activityCode) {
      setPageError(t('generer.missingCode'));
    } else {
      setPageError(null);
    }
  }, [activityCode, router]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/gemini/generate-affirmations`, {
        method: 'POST',
      ...
      if (!response.ok) {
        let errorData = { error: `${t('generer.apiError')} ${response.status}` };
        try {
            const textResponse = await response.text();
            try { errorData = JSON.parse(textResponse); } catch { errorData.error = textResponse; }
        } catch { /* ignore */ }
        throw new Error(errorData.error || `${t('generer.apiError')} ${response.status}`);
      }

      const data = await response.json();
      if (!data.affirmations || !Array.isArray(data.affirmations)) {
        throw new Error(t('generer.unexpectedFormat'));
      }
      ...
      alert(`${t('generer.generationError')}\n${error instanceof Error ? error.message : ''}`);
    } finally {
  ...
  const handleReload = async (affirmationId: number) => {
    ...
    try {
        const response = await fetch(`${API_BASE_URL}/api/gemini/make-harder`, {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ affirmation: currentAffirmation.text, explanation: currentAffirmation.explanation }),
            credentials: 'include',
         });
         ...
         setGeneratedAffirmations((prev) =>
            prev.map((aff) =>
              aff.id === affirmationId
                ? { ...aff, text: data.affirmation || aff.text, explanation: data.explanation || aff.explanation }
                : aff
            )
         );

    } catch (error: unknown) {
      console.error("Error making affirmation harder:", error);
      alert(error instanceof Error ? error.message : t('common.networkError'));
    } finally {
      setReloadingId(null);
      setIsGenerating(false);
    }
  ...
  const handleSaveToDb = async (affirmation: GeneratedAffirmation) => {
    if (!activityCode) {
        alert(t('generer.missingCodeSave'));
        return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        let errorMsg = responseData.warning || responseData.error || responseData.detail || `Error ${response.status}`;
        if (typeof responseData === 'object' && responseData !== null) {
            const fieldErrors = Object.entries(responseData)
                .map(([key, value]) => `${key}: ${Array.isArray(value) ? value.join(', ') : value}`)
                .join('; ');
             if (fieldErrors) { errorMsg = `${t('generer.validationError')} ${fieldErrors}`; }
        }
      ...
      if (responseData.warning) {
           alert(`${t('generer.savedWithWarning')}\n${responseData.warning}`);
      }

    } catch (error: unknown) {
      console.error('Error saving to database:', error);
      updateSaveStatus(affirmation.id, { isSaving: false, isSaved: false, error: error instanceof Error ? error.message : t('common.networkError') });
    }
  };
  ...
  const handleSaveEdit = (id: number) => {
    setGeneratedAffirmations((prev) =>
      prev.map((aff) =>
        aff.id === id ? { ...aff, text: editText, explanation: editExplanation } : aff
      )
    );
    setEditingId(null);
  };
                ...
                <div className="space-y-4">
                   <div>
                    <Label htmlFor={`edit-text-${affirmation.id}`} className="text-xl">
                      {t('generer.affirmationLabel')}
                    </Label>
                    <Textarea id={`edit-text-${affirmation.id}`} value={editText} onChange={(e) => setEditText(e.target.value)} className="mt-2 text-lg" rows={4}/>
                  </div>
                  <div>
                    <Label htmlFor={`edit-explanation-${affirmation.id}`} className="text-xl">{t('generer.explanationLabel')}</Label>
                  ...
                  <div className="flex justify-between items-start gap-4">
                    <div className="flex-1 min-w-0">
                      <div className="flex justify-between items-baseline mb-1">
                        <p className="font-medium text-xl">{t('common.affirmation')} {generatedAffirmations.findIndex(a => a.id === affirmation.id) + 1}</p>
                      </div>
                        ...
                        <div className="mt-3 pt-3 border-t border-gray-200">
                          <Button variant="link" onClick={() => toggleExplanationVisibility(affirmation.id)} className="p-0 h-auto text-sm text-blue-600 hover:text-blue-800 mb-2">
                            {isExplanationVisible
                              ? (<><EyeOff className="inline h-4 w-4 mr-1"/> {t('generer.hide')}</>)
                              : (<><Eye className="inline h-4 w-4 mr-1"/> {t('generer.show')}</>)
                      ...
                      {currentSaveStatus.error && (<p className="text-red-600 text-sm mt-2">{t('generer.saveError')} {currentSaveStatus.error}</p>)}
                    </div>
                    <div className="flex flex-col gap-2 items-end flex-shrink-0">
                      <Button
                        variant={currentSaveStatus.isSaved ? "default" : "secondary"}
                        size="sm"
                        onClick={() => handleSaveToDb(affirmation)}
                        disabled={currentSaveStatus.isSaving || currentSaveStatus.isSaved || isGenerating || editingId === affirmation.id}
                        className={`w-full justify-start text-left ${currentSaveStatus.isSaved ? "text-green-700" : ""}`}
                      >
                        {currentSaveStatus.isSaving
                          ? (<Loader2 className="h-4 w-4 mr-2 animate-spin" />)
                          : currentSaveStatus.isSaved
                          ? (<Save className="h-4 w-4 mr-2 text-green-700" />)
                          : (<Save className="h-4 w-4 mr-2" />)}
                        {currentSaveStatus.isSaving ? t('common.saving') : currentSaveStatus.isSaved ? t('generer.saved') : t('common.save')}
                      </Button>
                      <div className="flex gap-1">
                        <Button variant="ghost" size="icon" onClick={() => handleStartEdit(affirmation)} className="h-9 w-9 text-gray-600 hover:text-gray-900" title={t('common.edit')} disabled={isGenerating || currentSaveStatus.isSaving}>
                          <Pen className="h-5 w-5" />
                        </Button>
                        <DropdownMenu>
                          <DropdownMenuTrigger asChild>
                            <Button variant="ghost" size="icon" className="h-9 w-9 text-gray-600 hover:text-gray-900" title={t('generer.options')} disabled={isGenerating || currentSaveStatus.isSaving}>
                              <Settings className="h-5 w-5" />
                            </Button>
                          </DropdownMenuTrigger>
                          <DropdownMenuContent align="end">
                            <DropdownMenuItem onClick={() => handleDelete(affirmation.id)} className="text-lg text-red-600 focus:text-red-700 focus:bg-red-50">
                              {t('generer.deleteLocal')}
                            </DropdownMenuItem>
                          </DropdownMenuContent>
                        </DropdownMenu>
                      </div>
                      <Button variant="link" onClick={() => handleReload(affirmation.id)} className="text-blue-600 hover:text-blue-800 hover:underline text-base p-0 h-auto mt-1" disabled={isGenerating || currentSaveStatus.isSaving || editingId === affirmation.id}>
                        {reloadingId === affirmation.id ? (<Loader2 className="h-4 w-4 animate-spin" />) : t('generer.strengthen')}
                      </Button>
                    </div>
                  </div>
                </>
    ...
    return (
        <div className="min-h-screen bg-red-50 flex items-center justify-center p-8">
            <Card className="p-6 max-w-md text-center border-red-500">
                 ...
                 <h1 className="text-2xl font-bold text-red-700 mb-2">{t('common.error')}</h1>
        ...
        <div className="space-y-6">
          <div className="flex flex-col gap-4">
            <div className="flex-1">
              <Label htmlFor="question" className="text-xl font-medium mb-2 block">
                {t('generer.questionLabel')}
              </Label>
              <Input
                id="question"
                placeholder={t('generer.questionPlaceholder')}
                value={question}
            ...
            <div className="flex items-center justify-end gap-4">
              ...
              >
                {isGenerating && !reloadingId
                  ? (<><Loader2 className="mr-2 h-5 w-5 animate-spin" /> {t('generer.generating')}</>)
                  : t('generer.generate')}
              </Button>
            </div>
          </div>
          ...
          {generatedAffirmations.length > 0 && (
            <div className="mt-8 border-t pt-6">
              <h2 className="text-2xl font-semibold mb-4">{t('generer.generatedTitle')} ({generatedAffirmations.length})</h2>
              <div className="space-y-4">
...
export default function GenerateAffirmationPageWrapper() {
  return <Suspense><GenerateAffirmationPage /></Suspense>;
}
